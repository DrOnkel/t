...
// The library LedControl.h is here https://github.com/wayoda/LedControl
// installation guide is here http://arduino.cc/en/Guide/Libraries#.UwxndHX5PtY
...
pinMode (13,OUTPUT);
  ...
  /* and clear the display */
   lc.clearDisplay(0);
    ...
    // here we config the steppers parameters
  ...
  // config monitor output uart via USB 
     Serial.begin(115200);

Serial.println("And now we begin ");
...
void loop()
{
 ...
 // here we update the steppers positions
...
// every 250 ms we update indication and monitor output
...
void Pokaz(void)
{
 ...
 Serial.print("   Encoder and limit switch states= ");
  Serial.print(PINC );
  Serial.print("   \t X to be=");
 Serial.print(EncPosition*SCALE );
  Serial.print("   \tY to be=");
...
if(OldPinc&0x01   &&  (0x01&PINC)==0         ) // if Encoder Button pressed
{

 Regime= (++Regime)%4;  // turn the Regime

// and correct the relative Y speed
stepper2.setMaxSpeed(XMAXSPEED*XY_Limits[Regime][1]/XY_Limits[Regime][0]);

 

}
  if(OldPinc&0x04   &&  (0x04&PINC)==0     )  // if movement of encoder
{
